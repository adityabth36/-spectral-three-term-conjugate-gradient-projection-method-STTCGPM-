...
%
function [Tcpu,NF,Itr,NormF] = InerDFPI(fun,method,Switch,model,x0,x0_old,para) 
...
%                 w0 = Fy0-Fy0_old;
            ...
            case 'ISADFM'%Accelerated derivative-free method for nonlinear monotone equations with an application
                yk=Fz0_new-Fx0;
                betak=Fy0'*yk/dk*yk;
                thetak=Fy0'*dk/dk'*yk;  % 原始的分母是 norm(Fk0)^2  
                dk=-Fy0+betak*dk+thetak*yk;
            case'IDFPI'
                betak=0.01*norm(Fy0)/norm(dk);
                thetak=Fy0'*dk/norm(Fy0)^2;  % 原始的分母是 norm(Fk0)^2  
                dk=-Fy0+betak*dk+thetak*-betak*thetak*Fy0_old;
            otherwise
